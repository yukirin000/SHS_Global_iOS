...
-(void)copyFileDatabase
{
    ...
    _dbPath = [documentsDirectory stringByAppendingPathComponent:@"shs.db"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:_dbPath]) {
        debugLog(@"Database File Exsit");
    }else {
        NSString *resourceFolderPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"shs.db"];
